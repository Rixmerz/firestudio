...
type QueryValue = SimpleValue | SimpleValue[] | Record<string, unknown> | Record<string, unknown>[];
...
const QueryBuilder: React.FC<QueryBuilderProps> = ({ open, onToggle, onRunQuery, onClearQuery, hasActiveQuery }) => {
  const [queries, setQueries] = useState<QueryCondition[]>([{ field: '', operator: '==', value: '' }]);
  ...
  const updateQuery = (index: number, field: keyof QueryCondition, value: string) => {
    const newQueries = [...queries];
    newQueries[index][field] = value;
    setQueries(newQueries);
  };

  const parseValue = (value: string, operator: string): QueryValue => {
    // Handle array operators
    if (operator === 'in' || operator === 'not-in' || operator === 'array-contains-any') {
      try {
        const parsed = JSON.parse(value);
        return Array.isArray(parsed) ? (parsed as QueryValue) : value.split(',').map((v) => parseSimpleValue(v.trim()));
      } catch {
  ...
  const parseSimpleValue = (value: string): SimpleValue => {
        ...
        <Box sx={{ p: 2, backgroundColor: '#1a1a2e' }}>
          ...
          {queries.map((query, index) => (
            <Box key={index} sx={{ display: 'flex', gap: 1, mb: 1, alignItems: 'center' }}>
              ...
              />
              <IconButton size="small" onClick={() => removeQuery(index)} sx={{ color: '#666' }}>
                <DeleteIcon fontSize="small" />
              </IconButton>
            </Box>
          ))}

          <Button size="small" startIcon={<AddIcon />} onClick={addQuery} sx={{ mb: 2, fontSize: '0.75rem' }}>
            Add Condition
          </Button>
          ...
          <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
            ...
            <FormControl size="small" sx={{ width: 100 }}>
              <Select
                ...
                sx={{ fontSize: '0.8rem' }}
              >
                <MenuItem value="asc" sx={{ fontSize: '0.8rem' }}>
                  ASC
                </MenuItem>
                <MenuItem value="desc" sx={{ fontSize: '0.8rem' }}>
                  DESC
                </MenuItem>
              </Select>
            </FormControl>
