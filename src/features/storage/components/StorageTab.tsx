...
// File icon component
const FileTypeIcon: React.FC<FileTypeIconProps> = ({ item }) => {
  ...
  if (ct.includes('json') || ct.includes('javascript') || ct.includes('text'))
    return <CodeIcon sx={{ color: '#2196f3' }} />;
...
function StorageTab({ project, addLog, showMessage }: StorageTabProps) {
  ...
  const handleContextMenu = (e: React.MouseEvent<HTMLButtonElement>, item: StorageItem) => {
    e.preventDefault();
    e.stopPropagation();
    setSelectedItem(item);
  ...
  const handleUpload = async () => {
    if (!project) return;
    try {
      const result = await (isGoogle
        ? electron.googleStorageUploadFile({ projectId: project.projectId, storagePath: currentPath })
        : electron.storageUploadFile({ storagePath: currentPath }));
      if (result.success) {
        addLog?.('success', `Uploaded file`);
  ...
  const handleDownload = async (item: StorageItem) => {
    ...
    try {
      const result = await (isGoogle
        ? electron.googleStorageGetDownloadUrl({ projectId: project.projectId, filePath: item.path })
        : electron.storageGetDownloadUrl({ filePath: item.path }));
      if (result.success && result.url) {
        // Open the URL to trigger download
        electronService.openExternal(result.url);
        addLog?.('success', `Downloaded ${item.name}`);
        showMessage?.(`Download started for ${item.name}`, 'success');
      } else {
  ...
  const handleGetUrl = (item: StorageItem) => {
    closeMenu();
    setUrlFileItem(item);
    setDownloadUrl('');
    setUrlExpiration('7d');
  ...
  const generateSignedUrl = async () => {
    if (!urlFileItem || !project) return;
    setUrlLoading(true);
    try {
      const expiresInMs = EXPIRATION_OPTIONS.find((o) => o.value === urlExpiration)?.ms || EXPIRATION_OPTIONS[4].ms;
      const result = await (isGoogle
        ? electron.googleStorageGetDownloadUrl({
            projectId: project.projectId,
            filePath: urlFileItem.path,
            expiresInMs,
          })
        : electron.storageGetDownloadUrl({ filePath: urlFileItem.path, expiresInMs }));
      if (result.success && result.url) {
        setDownloadUrl(result.url);
        addLog?.('success', `Generated signed URL for ${urlFileItem.name}`);
      } else {
        showMessage?.(result.error || 'Failed to generate URL', 'error');
      }
    } catch (error) {
      showMessage?.((error as Error).message, 'error');
    } finally {
  ...
  const handleDelete = async (item: StorageItem) => {
    ...
    if (confirmed) {
      try {
        const deleteResult = await (isGoogle
          ? electron.googleStorageDeleteFile({ projectId: project.projectId, filePath: item.path })
          : electron.storageDeleteFile({ filePath: item.path }));
        if (deleteResult.success) {
          addLog?.('success', `Deleted ${item.name}`);
          showMessage?.(`Deleted ${item.name}`, 'success');
          loadFiles();
        } else {
          showMessage?.(deleteResult.error || 'Delete failed', 'error');
        }
      } catch (error) {
        showMessage?.((error as Error).message, 'error');
      }
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      // Note: Creating folders in Firebase Storage typically requires uploading a placeholder file
      // For now, we'll just show a message that this feature requires the full API
      showMessage?.('Folder creation requires uploading a file to that path', 'info');
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: 'background.default' }}>
        ...
        {loading ? (
          ...
          >
            ...
            <Typography sx={{ textAlign: 'center', maxWidth: 450, mb: 3 }}>
              Firebase Storage is not enabled for this project. Enable it in the Firebase Console to start uploading
              files.
            </Typography>
          ...
          <TableContainer>
            <Table size="small" stickyHeader>
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 600, bgcolor: 'background.default', color: 'text.primary' }}>
                    Name
                  </TableCell>
                  <TableCell
                    sx={{
                ...
                {items.map((item, i) => (
                  <TableRow
                    ...
                    sx={{
                      ...
                      bgcolor: selectedItem?.path === item.path ? 'action.selected' : 'transparent',
                    }}
                  >
                    ...
                    >
                      {item.size === 0 ? '—' : formatFileSize(item.size || 0)}
                    </TableCell>
                    <TableCell
                      sx={{
                        fontSize: '0.8rem',
                    ...
                    <TableCell sx={{ borderBottom: 1, borderColor: 'divider' }}>
                      {item.type === 'file' && (
                        <IconButton size="small" onClick={(e) => handleContextMenu(e, item)}>
      ...
      <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={closeMenu}>
        <MenuItem onClick={() => selectedItem && handleDownload(selectedItem)}>
          <ListItemIcon>
            <DownloadIcon fontSize="small" />
          </ListItemIcon>
          Download
        </MenuItem>
        <MenuItem onClick={() => selectedItem && handleGetUrl(selectedItem)}>
          <ListItemIcon>
        ...
        <Divider />
        <MenuItem onClick={() => selectedItem && handleDelete(selectedItem)} sx={{ color: 'error.main' }}>
          <ListItemIcon>
      ...
      <Dialog open={urlDialogOpen} onClose={() => setUrlDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth size="small" sx={{ mb: 2 }}>
            ...
            <Select value={urlExpiration} label="URL Expiration" onChange={handleExpirationChange}>
          ...
          {downloadUrl && (
            <>
              <Typography variant="caption" sx={{ color: 'success.main', display: 'block', mb: 1 }}>
                ...
                {EXPIRATION_OPTIONS.find((o) => o.value === urlExpiration)?.label || '7 days'})
              </Typography>
              <TextField
                fullWidth
                value={downloadUrl}
        ...
        <DialogActions>
          {downloadUrl && (
            <Button onClick={() => handleCopy(downloadUrl)} startIcon={<CopyIcon />}>
              Copy URL
            </Button>
          )}
